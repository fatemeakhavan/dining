...
const ReportTable = (props: IProps) => {
  const { handleClose, date, mealType } = props;
  ...
  let rows: IReservedReport[] = [];
  if (reportListHook.data?.length) {
    rows = reportListHook.data;
  }
  const emptyRows = rowsPerPage - Math.min(rowsPerPage, rows.length - page * rowsPerPage);

  return (
    <Dialog open={true} onClose={handleClose}>
      <DialogTitle>لیست گزارشات</DialogTitle>
      <DialogContent>
        <TableContainer>
          <Table className={classes.table} aria-label="simple table" style={{ direction: 'rtl', minWidth: 0 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell align="left">رستوران</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
