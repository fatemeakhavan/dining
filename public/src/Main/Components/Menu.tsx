...
export const MainListItems = (props: IProps) => {
  ...
  const { isAdmin, isMonitoring, handleClick } = props;
  return (
    <>
      {isAdmin ? (
        <React.Fragment>
      ...
      <ListItem button component={Link} to="/reserves" onClick={handleClick} selected={activeRoute === '/reserves'}>
        <ListItemIcon>
          <CourseIcon viewBox="0 0 350 350" />
        </ListItemIcon>
        <ListItemText primary="رزرو غذا" />
      </ListItem>
      <ListItem button component={Link} to="/qrcode" onClick={handleClick} selected={activeRoute === '/qrcode'}>
        <ListItemIcon>
          <CourseIcon viewBox="0 0 350 350" />
        </ListItemIcon>
