...
adminPanelAxiosInstance.defaults.baseURL = Manifest.adminPanel;
const AdminPanelQuery = async <T>(FetchConfig: {
  url: string;
  method: ERequest;
  data?: {
    [key: string]: any;
  ...
  return new Promise(async (resolve, reject) => {
    ...
    return await adminPanelAxiosInstance(request)
      .then((res) => {
        resolve((res.data || { data: 'OK' }) as IAdminPanelResult<T>);
      })
      .catch(async (error) => {
        ...
        if (error.response && error.response.data && error.response.status === 401) {
          ...
          if (refreshResult !== 'ERROR') {
            resolve(await AdminPanelQuery(FetchConfig));
            return;
          }
        }
