...
import useCreateServiceJenkinsAction from '../../../Hooks/ServiceJenkinsActions/useCreateServiceJenkinsAction';
...
const CreateServiceJenkinsAction = (props: IProps) => {
  const { handleClose, service, serviceGroupName } = props;
  const createServiceJenkinsActionHook = useCreateServiceJenkinsAction();
  ...
  const jenkinsFaRegexCheckErrorRef = useRef<HTMLInputElement>(null);
  const jenkinsValueValidationRegexRef = useRef<HTMLInputElement>(null);
  ...
  const [jenkinsInputs, setJenkinsInput] = React.useState<IServiceJenkinsInput[]>([]);
  const [jenkinsOutputs, setJenkinsOutputs] = React.useState<IServiceJenkinsOutput[]>([]);
  ...
  const addAction = () => {
    ...
    setError(null);
    createServiceJenkinsActionHook.mutate({
      serviceId,
      name,
      faDisplayName,
      faDescription,
  ...
  return (
    <Dialog open={true} onClose={handleClose} maxWidth={'md'}>
      <DialogTitle>اکشن جدید</DialogTitle>
      <DialogContent>
        ...
        <Select inputRef={actionTypeRef} defaultValue={'UNMANAGED_SETUP'} style={{ width: '100%', marginBottom: '15px' }}>
        ...
        <Box minWidth="100px">
          ...
          <Checkbox checked={enableChecked} onChange={handleEnableChange} color={'primary'} />
        </Box>
        ...
        <TableContainer style={{ marginBottom: '16px' }}>
          ...
          <Typography component="label">
            رجکس اعتبارسنجی (درصورتی که کاربر رجکس ورودی را رعایت نکند، این متن به عنوان دلیل خطا نمایش داده می شود)
          </Typography>
          <TextField
          ...
          {inputTypeChanged === 'VALUES' ? (
            <Container style={{ padding: 0 }}>
              <Typography component="label"> (مقادیر را با , از هم جدا کنید) مقادیر ورودی</Typography>
              <TextField
          ...
          <Box minWidth="100px">
            <Button
              ...
              disabled={createServiceJenkinsActionHook.isLoading}
            >
              افزودن ورودی جنکینز
            </Button>
          </Box>
      ...
      <DialogActions>
        {createServiceJenkinsActionHook.isLoading ? (
          <Box display="inline-block" marginTop="-50px" marginRight="50px" position="relative">
            <Spinner />
          </Box>
        ) : null}
        <Box minWidth="100px">
          <Button fullWidth color="primary" onClick={handleClose} disabled={createServiceJenkinsActionHook.isLoading}>
            انصراف
          </Button>
        </Box>
        <Box minWidth="100px">
...
export default CreateServiceJenkinsAction;
