...
const FoodsTable: React.FunctionComponent<IProps> = (props) => {
  const classes = useStyles();
  const { rows } = props;
  const [deleteFood, setDeleteFood] = useState<IFood | null>(null);
  const [editFood, setEditFood] = useState<IFood | null>(null);
  ...
  return (
    <>
          ...
          <TableBody>
            {(rowsPerPage > 0 && rows ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, index) => (
              <TableRow key={index} className="hover">
                ...
                <TableCell align="left">
                  <Box display="flex" alignItems="center">
                    <img width={100} src={row.pictureUrl} alt="food" />
                  </Box>
                </TableCell>
                <TableCell style={{ width: 200 }} align="left">
                  <IconButton size="small" title={'حذف'} onClick={() => setDeleteFood(row)}>
                    <TrashIcon />
                  </IconButton>
                  <IconButton size="small" title={'ویرایش'} onClick={() => setEditFood(row)}>
      ...
      {deleteFood ? <DeleteFood food={deleteFood} handleClose={() => setDeleteFood(null)} /> : null}
      {editFood ? <EditFood food={editFood} handleClose={() => setEditFood(null)} /> : null}
