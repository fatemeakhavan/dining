...
import { Box, Button, IconButton, makeStyles, MenuItem, TableFooter, Tooltip } from '@material-ui/core';
import { toPersinaDigit } from '../../../Helpers/Utils';
import { IDate } from '../../../Interfaces/Date.interface';
import { AlertIcon, EditIcon, InfoIcon, PenIcon, TrashIcon } from '../../../Assets/Svg';
            ...
            {(rowsPerPage > 0 && rows ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map((row, index) => (
              ...
              >
                ...
                <TableCell align="left">
                  {row.reserve?.food && row.reserve?.location ? (
                    ...
                    <Select
                      ...
                      style={{ width: '100%', marginBottom: '15px' }}
                      defaultValue={localStorage.getItem('DEFAULT_LOCATION:LOC')}
                      disabled={disabled || rows.findIndex((item) => item.reserve?.food) !== -1}
                    >
                ...
                <TableCell align="left">
                  {!row?.disabled ? (
                    <>
                      <Tooltip title={'ثبت درخواست'}>
                        <IconButton size="small" style={{ marginRight: 5 }} onClick={() => setEditRequest(row)}>
                          <PenIcon viewBox="0 0 81.471 77.829" />
                        </IconButton>
                      </Tooltip>
                    </>
                  ) : null}
                  <Tooltip title={'ثبت نظر'}>
                    <IconButton size="small" disabled={!row.reserve?.food} style={{ marginRight: 5 }} onClick={() => setAddComment(row)}>
                  ...
                  ) :    <Tooltip title={row.received ? 'دریافت شده' : 'دریافت نشده'}>
                    <IconButton size="small" style={{ marginRight: 5 }}>
                      <InfoIcon viewBox="0 0 1024 1024" />
                    </IconButton>
                  </Tooltip>}
                </TableCell>
              </TableRow>
